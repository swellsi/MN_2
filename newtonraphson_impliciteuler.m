...
function [ xfinal,xk,resk,it ] = newtonraphson_impliciteuler(x,fun,tol,itmax,m,n,constant,constant2)
it=1; xk=x; resk=max(abs(feval(fun,x,constant,constant2))); tolk=1;
while it<itmax & tolk>tol;
    x=xk(:,end);
    J=jacobian(fun,x,m,n,constant,constant2);
    b=-feval(fun,x,constant,constant2);
    [P,L,U]=PALU(J); % PJ=LU ---> P-1*L*U*Axk=-F(xk)=b ---> LUAxk=Pb=c
    c=P*b; d=L\c; Axk=U\d;
    xk=[xk Axk+x];
    it=it+1; tolk=max(abs((xk(:,end)-xk(:,end-1))));
    resk=[resk; max(abs(feval(fun,xk(:,end),constant,constant2)))];
end
xfinal=xk(:,end);
end

function DF = jacobian(F,x,m,n,constant,constant2)
...
I=eye(m);
for i=1:n
    DF(:,i)=1/h*(feval(F,x+I(:,i)*h,constant,constant2)-feval(F,x,constant,constant2));
end
end
